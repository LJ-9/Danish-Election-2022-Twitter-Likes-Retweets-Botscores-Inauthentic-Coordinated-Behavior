...
printf "${GREEN}
        So, you want to look up some like and retweet counts
        of old tweets, huh?${RED}
...
case "$ynchoice" in
  ...
  n|N ) printf "
    ...
    if [ -d $CHOICE ]; then
      printf "${GREEN}
        Great, I see $CHOICE
        and will look up tweeeeeeeets!
        This may take a bit!${NORMAL}\n"
      export CHOICE
      python3 -c "import os; from resources.fv22 import *;\
                  latercounts(os.getenv('CHOICE'))"
      printf "${MAGENTA_BLINK}\n  DONE!${NORMAL}\n\n"
    else
