...
printf "${GREEN}
        So, you want to look up some retweeting users, ai?
...
"${NORMAL}
    SAVEPATH=./Late-retweeting-users/$line.jsonl
    if [ -f $SAVEPATH ]
    then
      echo "Tweet ID: $line -- File exists"
    else
      echo "Tweet ID: $line"
      twarc2 --bearer-token $BEARER retweeted-by $line $SAVEPATH
    fi
